...
export function generateStyleClass(source: string): string {
  return `s${nextScopedStyleNumber(source)}`;
}
...
/**
 * Find the next available numeric suffix for scoped tokens (s1, s2, ...).
 * Scans both block labels and class targets so IDs never collide.
 */
function nextScopedStyleNumber(source: string): number {
  const used = new Set<number>();

  // mkly content annotations: {... .sN}
  for (const m of source.matchAll(/\{\.s(\d+)\}/g)) {
    used.add(parseInt(m[1], 10));
  }

  // Block headers with labels: --- kit/block: sN
  for (const m of source.matchAll(/^---\s+[\w/]+:\s*s(\d+)\s*$/gm)) {
    used.add(parseInt(m[1], 10));
  }

  // Style block labeled selectors: kit/block:sN
  for (const m of source.matchAll(/^\s*[\w]+\/[\w]+:s(\d+)\s*$/gm)) {
    used.add(parseInt(m[1], 10));
  }

  // Verbatim HTML class attributes containing sN
  for (const m of source.matchAll(/\bclass="([^"]*)"/g)) {
    for (const token of m[1].matchAll(/\bs(\d+)\b/g)) {
      used.add(parseInt(token[1], 10));
    }
  }
