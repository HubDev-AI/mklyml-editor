...
import { findBlockByOriginalLine, cleanHtmlForReverse } from './reverse-helpers';
...
describe('end-to-end: compile → reverse → findBlockByOriginalLine', () => {
  // Compile the source once — full HTML includes meta/script tags for preamble round-trip
  ...
  for (const bt of blockTests) {
    test(`round-trip: ${bt.type} maps back correctly`, () => {
      ...
      // Reverse compile the full HTML — engine reconstructs preamble from embedded meta/script tags
      ...
      const resultLine = findBlockByOriginalLine(reversed, originalLine, bt.class);
      expect(resultLine).not.toBeNull();

      // Verify the found line is the right block type
      const resultLines = reversed.split('\n');
  ...
  test('newsletter/intro does NOT round-trip to newsletter/item', () => {
    ...
    const resultLine = findBlockByOriginalLine(reversed, introLine, 'mkly-newsletter-intro');
    expect(resultLine).not.toBeNull();
  ...
  test('first core/card round-trips to first core/card', () => {
    ...
    const result1 = findBlockByOriginalLine(reversed, cardLines[0], 'mkly-core-card');
    const result2 = findBlockByOriginalLine(reversed, cardLines[1], 'mkly-core-card');
  ...
  test('first newsletter/item round-trips to first newsletter/item', () => {
    ...
    const result1 = findBlockByOriginalLine(reversed, itemLines[0], 'mkly-newsletter-item');
    const result2 = findBlockByOriginalLine(reversed, itemLines[1], 'mkly-newsletter-item');
