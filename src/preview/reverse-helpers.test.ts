...
describe('ensurePreamble', () => {
  test('preserves original preamble', () => {
    const original = `--- use: core
--- use: newsletter
--- meta
version: 1
...
Hello
`;
    const result = ensurePreamble(reversed, original);
    expect(result).toContain('--- use: newsletter');
    expect(result).toContain('--- meta');
    expect(result).toContain('version: 1');
    expect(result).toContain('--- core/header');
    expect(result).toContain('Hello');
  });

  test('keeps body blocks from new source', () => {
    const original = `--- use: core
...
`;
  ...
  test('handles source with no preamble', () => {
    const original = `--- core/header
title: Test
`;
    const reversed = `--- core/header
title: Test
`;
    const result = ensurePreamble(reversed, original);
    expect(result).toContain('--- core/header');
  ...
  test('handles source with only preamble', () => {
    const original = `--- use: core
--- meta
version: 1
`;
...
`;
  ...
  test('preserves preset in preamble', () => {
    const original = `--- use: core
--- meta
version: 1
--- preset: default
...
`;
    ...
    expect(result).toContain('--- meta');
    expect(result).toContain('version: 1');
    expect(result).toContain('--- preset: default');
    expect(result).toContain('spacing: compact');
    expect(result).toContain('Edited Title');
  });

  test('preserves theme in preamble', () => {
    const original = `--- use: core
--- meta
version: 1
--- theme: brand
...
`;
  ...
  test('preserves full preamble with all directive types', () => {
    const original = `--- use: core
--- use: newsletter
--- meta
version: 1
